#ifndef GAZEBO_ROCK_SONAR_RENDER_HH
#define GAZEBO_ROCK_SONAR_RENDER_HH
...
public:
    ...
    gazebo::Sonar last_sonar;

    RockSonar()
	    : bin_count(500)
	    , beam_count(0)
	    , beam_width(gazebo::Angle::fromRad(0.0))
	    , beam_height(gazebo::Angle::fromRad(0.0))
	    , speed_of_sound(gazebo::Sonar::getSpeedOfSoundInWater())
	    , beam_cols()
	    , last_sonar()
    {}

    RockSonar(uint32_t bin_count, uint32_t beam_count, gazebo::Angle beam_width, gazebo::Angle beam_height)
	    : bin_count(bin_count)
    ...
    */
    gazebo::Sonar simulateSonar(const std::vector<float>& bins, float range);
