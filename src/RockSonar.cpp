#include "forward_looking_sonar_gazebo/RockSonar.hh"
...
void RockSonar::decodeShader(const cv::Mat& cv_image, std::vector<float>& bins) {
...
gazebo::Sonar RockSonar::simulateSonar(const std::vector<float>& bins, float range) {
    gazebo::Sonar sonar;
    sonar.time = gazebo::Time::now();
    sonar.bin_duration = gazebo::Time::fromSeconds(getSamplingInterval(range) / 2.0);
...
void RockSonar::convertShader(cv::Mat& cv_image, std::vector<float>& bins) {
...
void RockSonar::applySpeckleNoise(std::vector<float>& bins, float mean, float stddev) {
    // produce speckle noise using a gaussian distribution
    unsigned long seed = gazebo::Time::now().toMicroseconds();
...
float RockSonar::sigmoid(float x) {
    float beta = 18, x0 = 0.666666667;
    float t = (x - x0) * beta;
    return (0.5 * tanh(0.5 * t) + 0.5);
}

float RockSonar::getSamplingInterval(float range) {
    float travel_time = range * 2.0 / speed_of_sound;
    return travel_time / bin_count;
}

void RockSonar::applyAdditionalGain(std::vector<float>& bins, float gain) {
